...
#also, if it uses a cache, could some files be missing from the scan?
...
def walk(directory, exclude=[], ignore_symlinks=False, include_symlinks=[]):
  for fn in ld(directory): #todo: we should recursively pass the current parts list instead of using PurePath(p).parts for every p
    ...
    if os.path.isfile(p):
      ...
      if not (ignore_symlinks and os.path.islink(p) and not any(parts[-len(x):] in x for x in include_symlinks)): #todo: is this right?
        if not any(parts[-len(x):] in x for x in exclude): #this is really dirty but i don't know of a better solution do excludes how I want
          yield from walk(p, exclude, ignore_symlinks, include_symlinks)
...
def n(p, fn, i_p, i_f, ignore_symlinks=False, include_symlinks=[]):
  for path in i_p:
    for p, fn in walk(path, x_paths, ignore_symlinks):
...
try: #can we make this code even less redundant? we could with -r and -R, but i don't want to make this too complicated.
  if args.r:
    for pf in i_files:
      p, spec = os.path.split(pf)
      if p:
        n(p, spec, [p], [spec], ignore_symlinks=True, include_symlinks=[p]+i_paths)
      else:
        n(p, spec, i_paths or ["."], i_files, ignore_symlinks=True, include_symlinks=i_paths)
  elif args.R:
    for pf in i_files:
      p, spec = os.path.split(pf)
      if p:
