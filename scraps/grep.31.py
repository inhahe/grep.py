...
parser.add_argument("--set-colors", nargs=4, metavar="color", help="provide four color names to set filenames, line numbers, match contents, and error messages to."
...
try:
  ...
  print(f"{red}Regex pattern error: {neutral}{", ".join(e.args)}")
  sys.exit()
...
def ld(directory=None):
  try:
    ...
    print(f"{red}{"permission denied" if type(e) is PermissionError else "i/o error"}: {neutral}{p}")
    return []
  else:
    return r
...
def prn(p, ln, s):
  ...
  if args.line_numbers:
    print(f"{green}{p}{gray}:{red}{ln}{gray}:{neutral}{s2}")
  else:
    print(f"{green}{p}{gray}:{neutral}{s2}")
