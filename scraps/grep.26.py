...
#use system calls to do colors on windows instead of using colorama so we don't have any requirements. nevermind, i don't think python has any out-of-the-box support for windows api calls.
#oops, --dotall only returns one result
#fix some things use x+y and some things use f"{x}{y})", make them all use f strings
#remove all the "."'s to make it compatible with linux. nevermind, os.listdir(".") apparently works on linux.
