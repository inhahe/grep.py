...
#option to use colors but don't set them, use the ones read from config file?
...
parser.add_argument("--allow-match-colors", action="store_true", help="show ANSI colors if they exist in the match text, but not other escape codes")
...
if os.path.isfile(cf):
  try:
    ...
    fcolors = dict(config.items("colors"))
    print(("here 1",fcolors)) #debug
    usecolors = config["DEFAULT"].getboolean("use_colors")
    allowmatchcolors = config["DEFAULT"].getboolean("allow_match_colors")
...
try:
  ...
  config["DEFAULT"]["use_colors"] = str(not args.no_color)
  config["DEFAULT"]["allow_match_colors"] = str(allowmatchcolors)
