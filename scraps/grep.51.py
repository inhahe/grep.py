...
# maybe we should wrap prn in a try and print error printing line (with a line number if there is one) if there's an error
#showing match results from binary files is problematic. it looks really messy, the lines tend to be really long, 
# and it can do weird things to the terminal.
...
sparts = set()
def walk(directory, parts, ignore_symlinks=False): #maybe we should make x_paths and i_paths explicitly passed here even though they're 
  global sparts
  if not parts in sparts:
    for fn in ld(directory):                         # never going to be changed
      p = os.path.join(directory, fn)
      if os.path.isfile(p):
        yield (p, fn)
      elif os.path.isdir(p):
        parts2 = parts+[fn]
        if not (ignore_symlinks and os.path.islink(p) and not any(parts2[-len(x):] == x for x in i_paths)): #todo: is this right?
          if not any(parts2[-len(x):] == x for x in x_paths): #this is really dirty but i don't know of a better solution do excludes 
            yield from walk(p, parts2, ignore_symlinks)       # how I want
  sparts.add(parts)
...
def n(p, fn, i_p, i_f, ignore_symlinks=False, include_symlinks=[]):
  for path in i_p:
    for p, fn in walk(path, [path], ignore_symlinks):
...
try: 
  if args.r or args.R:
    for pf in i_files:
      ...
      if p:
          print(f"here 1: {p=}")#debug
          n(p, spec, [p], [spec], ignore_symlinks=args.r, include_symlinks=[p]+i_paths)
      else:
        print(f"here 2: {p=}")#debug
 #       n(p, spec, i_paths or ["."], i_files, ignore_symlinks=args.r, include_symlinks=i_paths)
        n(".", spec, i_paths or ["."], i_files, ignore_symlinks=args.r, include_symlinks=i_paths)
  else: #can we make the following code simpler?
    if i_paths: #todo: in any case, we should change this so that we only walk once for all teh file specs  
      for spec in i_files:
