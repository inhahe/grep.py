...
#add an option to permanently set colors
...
parser.add_argument("--set-colors", nargs=5, metavar="color", help="provide five color names to set the colors of filenames, colons, line numbers, match contents, and error messages to."
...
if use_colors:
  colors = dict(zip("black, darkred, darkgreen, darkyellow, darkblue, darkmagenta, darkcyan, lightgray, gray, red, green, yellow, blue, magenta, cyan, white, neutral".split(", "), 
                     list(f"\033[{x}m" for x in range(30, 38)) + list(f"\033[1;{x}m" for x in range(30, 38))+["\033[0m"]))
else:
  colors = dict(zip("black, darkred, darkgreen, darkyellow, darkblue, darkmagenta, darkcyan, lightgray, gray, red, green, yellow, blue, magenta, cyan, white, neutral".split(", "), 
                     [""]*17))

d = os.path.basename(os.path.abspath(__file__))
cf = os.path.join(d, "grep.py.colors.conf")

if args.set_colors:
  open(cf, "w").write(" ".join(args.set_colors))
  fncolor, coloncolor, lncolor, matchcolor, errcolor = args.set_colors
else:
  if os.path.isfile(cf):
    fncolor, coloncolor, lncolor, matchcolor, errcolor = open(cf).read().split()
  else:
    fncolor, coloncolor, lncolor, matchcolor, errcolor = colors["green"], colors["gray"], colors["red"], colors["neutral"], colors["red"]
