...
#i don't think absolute paths will currently work.
#colorize output
...
def process(p):
  ...
  if p not in s:
    try:
      ...
      print("could not open: " + p) #todo: return something ins
    else:
      if not args.dotall:
...
try:
  s = set()
  if args.r:#todo; make the code shorter
    if i_paths:
      for pf in i_files:
        p, spec = os.path.split(pf)
        if p:
          for p, fn in walk(p, x_paths):
            if any(fnmatch(fn, pat) for pat in i_files) and not any(fnmatch(fn, pat) for pat in x_files):
      ...
      for pf in i_files:
        ...
        if p:
          ...
          for p, fn in walk(".", x_paths):
            if any(fnmatch(fn, pat) for pat in i_files) and not any(fnmatch(fn, pat) for pat in x_files):
              process(p.removeprefix(".\\"))

  else: #this part seems to work. todo: make less code here. most of this is redundant.
    if i_paths:
      for spec in i_files:
        p, fspec = os.path.split(spec)
        if p:
          for fn in ld(p):
            if fnmatch(fn, fspec) and not any(fnmatch(fn, spec2) for spec2 in x_files):
              pf = os.path.join(p, fn)
              if os.path.isfile(pf): 
                process(pf)
        else:
          for path in i_paths:
            for fn in ld(None if path=="." else path):
              if any(fnmatch(fn, pat) for pat in i_files) and not any(fnmatch(fn, pat) for pat in x_files):
                p = fn if path=="." else os.path.join(path, fn)
                if os.path.isfile(p): 
                  process(p)
    else: #this part seems to work. #todo: make less code here. most of it is redundant.
      for f in i_files:
        p, spec = os.path.split(f)
        if p:
          for fn in ld(p):
            if fnmatch(fn, spec) and not any(fnmatch(fn, spec2) for spec2 in x_files):
              pf = os.path.join(p, fn)
              if os.path.isfile(pf): 
                process(pf)
        else:
