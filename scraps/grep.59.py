...
#make notifications more consistent, some start with a lowercase letter and some start with an uppercose
#if there's an error passing colors, show the error message using the colors specified in the conf file. but that will be really tricky. 
# because we're also showing errors reading the config file in whatever colors were passed to --set-color. and one or the other has to be read first.
# we should actually change it to the other way around, because an error passing colors is a lot more likely.
...
if args.set_colors:
  if args.set_colors == []:
    ...
    if invalidcolors:
      print(f"{c.errcolor}error: {c.normalcolor}invalid color(s) passed: {', '.join(invalidcolors)}{colors['default']}")
      quit()
    else:
