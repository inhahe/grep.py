...
context_buffer.extend([None]*(before_context or 0))
printing_context = False
tracking_context = False
lines_since_match = None
          ...
          for line in inf:
            if before_context or after_context:
              if printing_context:
                ...
                if tracking_context:
                  if regexc.search(line):
                    for l in list(context_buffer)[:lines_since_match - after_context + 1]:
                      print(p+":2: "+decode(l))
                    printing_context = True
                    lines_since_match = 1
                    print(p+":3: "+decode(line))
                  else:
                    lines_since_match += 1
                    if lines_since_match > after_context + before_context:
                      tracking_context = False
                      print("--")
                else:
                  if regexc.search(line):
                    ...
                    lines_since_match = 1
                    for l in list(context_buffer)[:before_context:]:
                      print(p+":4: "+decode(l))
